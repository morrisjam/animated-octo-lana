...
Do not define `VITE_APP_BUILD` in the Cloudflare Pages dashboard. During a Pages build,
`vite.config.ts` derives the public game build ID from Cloudflare's injected
`CF_PAGES_COMMIT_SHA` and embeds that exact value into the application bundle. A manually
configured `VITE_APP_BUILD` is accepted only when it equals the provider commit SHA; a stale
or conflicting value fails the build. The offline provider-config audit still takes an
explicit `VITE_APP_BUILD=<release_sha>` in its input file so it can prove the intended API,
web, and allowlist identity before provider contact.

### Pages release attestation

The safe rollout does not have a Cloudflare API token or Pages deploy hook. Each
canary/production web target must instead expose `/release.json`, generated by the build from
the provider commit currently serving users:
...
The Vite build emits both `release.json` and the Cloudflare `_headers` rule that applies
`Cache-Control: no-store`. The endpoint must return HTTP 200, must not redirect, and derives
`releaseSha` from `CF_PAGES_COMMIT_SHA` rather than a workflow input or manually mutable
value. Configure the full `/release.json` URL and independently expected hostname in the
`WEB_*_RELEASE_ATTESTATION_URL` and `WEB_*_EXPECTED_HOSTNAME` GitHub variables. A missing or
mismatched endpoint blocks the rollout. Because this repository has no web-provider
deployment credential, an API rollback remains paused until Pages also serves and attests
the rollback SHA.
...
Render supplies `RENDER_GIT_COMMIT`; the API reports it as `releaseSha` from `/health` and `/readyz`. `/health` is a database-free liveness probe and is excluded from durable SLO sampling, so provider polling cannot keep Neon awake. `/readyz` deliberately checks PostgreSQL and requires every migration filename and SHA-256 digest shipped by the running release to match the applied prefix. The hosted alpha profile must set `MIGRATION_ALLOW_FORWARD_COMPATIBLE_SUFFIX=true`; only then may an older rollback release accept additional applied rows, and only as a fully checksummed suffix after its exact known prefix. A missing, reordered, legacy-null, or modified row still returns `503`. This is reserved for the bounded safe-rollout gate and operator diagnostics. The offline provider audit blocks any `RENDER_HEALTH_CHECK_PATH` other than `/health` and requires the forward-compatible rollback setting. The safe rollout gate also checks `DEPLOYMENT_ENVIRONMENT`, `DEPLOYMENT_DATABASE_ID`, database connectivity, the actual applied migration head, PostgreSQL matchmaking coordination, a runtime namespace equal to the rollout target, and the admin-gated `gw.runtime-security-posture.v1` contract before it resumes matchmaking. Configure GitHub repository variables `API_CANARY_DATABASE_ID` and `API_PRODUCTION_DATABASE_ID` to match the corresponding Render values. Configure `API_CANARY_EXPECTED_HOSTNAME` and `API_PRODUCTION_EXPECTED_HOSTNAME` independently of the secret API base URLs; the gate rejects non-HTTPS hosted URLs, hostname mismatches, URL credentials, and redirects before sending the operations key.

The Pages-derived build ID, `VITE_RULESET_VERSION`, and `VITE_BALANCE_PROFILE_ID` must match
the API's durable session/verifier contract. Ranked queue entry fails closed when they do
not. Before safe rollout, keep both the candidate and rollback SHA in the build allowlist;
the admin-only deployment check verifies each exact SHA without exposing the list.
...
This runner starts an isolated production-mode API, first closed and then with generated exact account/build allowlists. It proves signed-session enforcement, public status redaction, admin-gated exact-build membership, stale-SHA rejection, queue and private-room admission, and participant-bound 60-second TURN credentials. It refuses non-loopback databases before contact, never contacts the relay, constrains its report to `apps/api/build-artifacts`, redacts generated values, and removes its namespaced records. It is also part of `alpha:local-gate`, `alpha:local-integration`, and CI.

CI runs the deployable production bundle in simulated Pages mode using `github.sha`; the
build fails unless `release.json`, its no-store `_headers` rule, and the compiled game
JavaScript all carry that same exact SHA. CI preserves that provider-equivalent bundle,
then builds a separate loopback-only instrumented bundle for the two-browser ranked smoke.
The provider build rejects the local bridge, and an exact-identity smoke binds the preserved
web attestation, API readiness SHA, applied migration identity, and private build allowlist.
The complete local integration runner now rehearses the same binding before its browser and
failure-recovery stages. Its v5 report records the repository SHA and dirty-worktree state;
only a clean run can be treated as deployable evidence, while dirty runs remain useful local
contract diagnostics.
CI also runs the production bundle budget and production-root
`npm run alpha:visual-smoke`, the production controlled-access gate, durable auth-rate and
route-ownership smokes, plus `npm run api:smoke:ranked-online`,
`npm run api:smoke:ranked-authoritative-forfeit`,
`npm run api:smoke:database-interruption`, `npm run api:smoke:matchmaking-restart`, and
`npm run api:smoke:matchmaking-multi-instance` against disposable local services before
release evidence is accepted. The bundle gate measures the emitted initial graph and
requires optional tools to remain lazy. The visual gate blocks external requests, requires
bundled replay JSON and WebGL rendering, exercises Replay Review and pause/resume first-load
behavior, and archives action-marker screenshots. The service gates prove atomic auth
boundaries, account privacy, valid proof replay, tamper rejection, durable authenticated
heartbeat handling, pool reconnection, checkpoint/restore, reconnect replay protection,
signaling recovery, pending proof-backed settlement across sequential API processes, and
shared queue/session/transport/drain state across two simultaneously live API processes.
The multi-instance gate terminates only its explicitly named API backends while a ranked
session is active, requires replacement backend PIDs, and proves both API PIDs and
cross-instance control-plane operations survive. The API health response reports only the
database target class (`local`, `remote`, or `unknown`), never connection details; smoke
commands refuse anything except `local` by default. Set `ALLOW_REMOTE_DATABASE_SMOKE=1`
only while intentionally running the same commands against an isolated staging database
during release rehearsal. This is not permission to target production or skip the
production drain because only the current simulator verifier is loaded and the legacy HTTP
frame relay remains process-local.
...
2. Trigger the safe rollout workflow with target `canary`, the exact 40-character release SHA, and a different known-good rollback SHA that is currently deployed on the target. It requires both SHAs to be reachable from `master` with successful exact-SHA CI, installs both releases from their own lockfiles, requires the exact candidate's strict `ai:alpha-readiness` gameplay gate to pass locally, and runs `gw.rollback-schema-compatibility.v2`. That proof starts the rollback API before the upgrade, applies candidate migrations, reruns the rollback release's own migration command, and proves the same rollback API can still serve account/profile/ranked writes against the upgraded schema. The workflow then proves the deployed rollback API and web attestation satisfy the baseline readiness contract before any drain or deploy call. It pauses new queue joins, closes waiting tickets, and waits for active online sessions to finish before calling Render with `ref=<release_sha>`:
   - `.github/workflows/safe-rollout.yml` with target `canary`.
   - TURN with short-lived credentials, alpha allowlists, the `64` resident-ticket ceiling, readiness identity, SLO checks, operations checks, production signed-auth posture, and official Steam-verifier configuration are mandatory rather than operator-selectable inputs.
...
5. Confirm the drain gate reports zero queued tickets and zero active sessions. Queue/session/reconnect state, heartbeat timestamps, and low-volume WebRTC signaling are durable in PostgreSQL; accepted heartbeat pulses persist the coordinated runtime snapshot, while gameplay frames travel peer-to-peer or through TURN rather than through the API process. The legacy HTTP frame relay is process-local and is not an alpha transport.
   - If the deploy hook times out or returns an indeterminate result, the workflow fails with matchmaking still drained. Verify the provider's actual release state before manually resuming or retrying; do not assume the candidate was not accepted.
6. Render runs `npm run api:migrate` before starting the new API process.
   - Migration `031_ranked_input_commitments.sql` is an additive expand step: the prior API ignores its new table and proof column, while the participant-side projection remains nullable during the rolling rollback window.
   - Migration `032_steam_ticket_exchange_replay_guard.sql` is also additive: the prior API ignores the new fingerprint table, while the candidate API uses its primary key to make a successful Steam Web API ticket exchange one-time across processes. Account deletion sets the optional account reference to null and retains the replay claim.
   - Migration 031 must not be the first deployment that introduces forward-compatible migration readiness. First ship and rehearse a closed-mode bootstrap release that contains the suffix-aware migration/readiness behavior but still has migration head 030. Only after that exact build is the deployed known-good rollback may a later release add migration 031. The compatibility workflow will intentionally fail if the selected rollback SHA cannot run against the candidate suffix.
...
8. Validate:
   ...
   - `https://api.gravitywell.space/readyz` reports the exact release SHA, expected database identity/environment, matching runtime namespace, `postgres_advisory_lock_fenced_v2` coordination, all 32 release-source-matched migrations through `032_steam_ticket_exchange_replay_guard.sql`, and `replayBlobDurable: true`
   - the credentialed deployment report records `gw.runtime-security-posture.v1`, `runtimeSecurityConfigurationReady: true`, and `steamTicketVerifierConfigured: true`; this proves safe configuration but does not replace the real Steam account exchange rehearsal
   ...
   - a production-timed ranked rehearsal returns `proof.inputAttestation.status = "match_verified"`, includes both assigned sides, covers every verified proof frame, and gives each participant an `observationRatio` of at least `0.25`; accelerated local evidence cannot satisfy this hosted timing check
