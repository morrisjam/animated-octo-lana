...
- `SLO_ADMIN_KEY` optional admin key required by SLO summary endpoint.
...
- `API_SLO_ADMIN_KEY` optional for deploy health-gate SLO checks.
...
- API records per-request samples in `service_slo_request_samples` for availability/error/latency SLO calculations.
