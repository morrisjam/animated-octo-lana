...
The runner creates a disposable local database, applies the rollback commit's migrations, starts that commit's API and writes representative account, profile, ranked-season, queue, and session state, applies the candidate migrations, then restarts the same rollback API and repeats the probe. The ranked queue write includes the complete build, ruleset, balance-profile, platform, and character contract required by ranked matchmaking. Rollback source and candidate migrations are read from exact Git archives; candidate migrations execute with the candidate installation's runtime dependencies but from the committed migration snapshot, so working-tree newline or checkout filters cannot alter the release evidence. Failure paths independently remove every temporary archive/snapshot and still emit a failed JSON report. Evidence schema `gw.rollback-schema-compatibility.v1` records both exact SHAs, both migration-chain digests, candidate dirty state, every documented compatibility exception, and all four phases. It refuses remote databases and contacts no hosted service. CI and safe rollout require a clean candidate and bind the report to the exact release/rollback pair; a local dirty-tree pass is diagnostic evidence only.
...
This local-only runner first builds a provider-equivalent immutable web artifact from the repository HEAD, then builds the separate instrumented production-root client used by the browser harness with the same exact Git SHA. It verifies the immutable artifact's `release.json`, no-store header, and compiled SHA; starts the API with that same release SHA, the checksummed forward-compatible migration contract, and the exact client-build allowlist; and runs `gw.exact-release-identity-smoke.v1` before accepting later service evidence. Both the generic WebRTC harness and each production-root ranked client must report that SHA. The remaining stages cover rollback-only ranked season transition, ranked proof consensus, archived Master-region immutability, server-authoritative forfeit, real-browser WebRTC rollback/reconnect, two-client lifecycle/script-stall recovery, a one-second isolated-client rollback soak, and a complete two-browser ranked match through the production root. The production-root match replaces the live WebRTC channel while each peer retains a speculative unacknowledged input tail, then requires same-round checkpoint agreement, exactly one transport-generation advance, tail drain, proof consensus, replay persistence, database interruption recovery, process replacement, and concurrent-instance durability.

The TURN variant owns an ephemeral `coturn/coturn:4.6.3` container and fails unless initial, replacement, isolated two-client, and production-root connections all report relay paths with short-lived credentials. The runner refuses a non-loopback `LOCAL_DATABASE_URL`, does not contact Neon or another hosted application provider, cleans up only services it started, and writes redacted `gw.local-alpha-integration.v5` summaries as `report-direct.json` or `report-relay.json` plus a latest-run `report.json` compatibility copy under `apps/api/build-artifacts/local-alpha-integration`. Its `releaseIdentity` section records the exact SHA, dirty-worktree status, and whether the run can count as deployable evidence. Dirty runs are diagnostic only. API, preview, browser, exact-identity, and coturn evidence is retained in the same directory. Docker, Git, and Chrome, Edge, or Chromium are required.

`LOCAL_ALPHA_SKIP_BUILD=1` skips both builds for fast topology repetition only after the current immutable and instrumented artifacts have passed; both attestations are still revalidated. Ports default to API `28787`, web `25191`, controlled-access `28789`, restart `28791`, instances `28792-28793`, TURN `23478`, and relay range `49160-49179`. Each has a corresponding `LOCAL_ALPHA_*_PORT` override. Set `LOCAL_ALPHA_MANAGE_POSTGRES=0` to require an already-running local database instead of allowing Docker startup.
...
Migration `032_steam_ticket_exchange_replay_guard.sql` stores one durable SHA-256 fingerprint for each successfully exchanged non-development Steam Web API ticket. The claim is committed atomically with account resolution before a signed Gravity Well session is issued; the raw ticket is never stored. A duplicate or case-varied replay returns `401 steam_ticket_already_exchanged` and requires a newly acquired Steam ticket.
...
- `POST /auth/steam/exchange` verify a fresh `GetAuthTicketForWebApi` ticket with Steam, durably claim it once, link/create the account, and issue a signed session.
