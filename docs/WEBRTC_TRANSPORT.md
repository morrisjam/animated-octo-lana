...
The first command proves a direct ICE path. Its smoke-only peer configuration keeps STUN and host discovery candidates but removes every TURN URL, so an available runner-local relay cannot silently satisfy direct-path evidence. The harness retains the unmodified API-issued configuration separately and still verifies that its TURN fallback is time-limited. The TURN variant keeps every issued server, starts ephemeral local coturn, gives the API a random REST shared secret, requests short-lived account-scoped credentials, forces `iceTransportPolicy: relay`, and rejects any initial, recovered, or isolated two-client connection that does not report a relay candidate pair. These constraints apply only to local/CI proof peers; ordinary production clients continue to receive and use the full server-issued ICE configuration. Both commands validate a loopback database target, start or reuse local Docker PostgreSQL, build and serve the production client, bind the provider-equivalent artifact and instrumented browser artifact to the same API release SHA, checksummed migration history, and private client-build allowlist, require both browser harnesses to report that SHA, run ranked/forfeit/restart/multi-instance checks plus a one-second isolated-client rollback soak, and clean up only resources they started. Set `LOCAL_ALPHA_SKIP_BUILD=1` for repetition only after both current web artifacts have passed. The machine needs Docker, Git, and Chrome, Edge, or Chromium. A dirty-worktree v5 report is deliberately marked diagnostic rather than deployable evidence.
...
Set `BROWSER_EXECUTABLE_PATH` when Chrome, Edge, or Chromium is outside a standard install location. `WEBRTC_BROWSER_SMOKE_URL`, `WEBRTC_BROWSER_SMOKE_API_BASE_URL`, and `WEBRTC_BROWSER_SMOKE_TIMEOUT_MS` override the defaults. `WEBRTC_BROWSER_SMOKE_FORCE_RELAY=1` requires relay policy, relay candidate pairs, relay availability, and time-limited credentials across both phases. The direct and forced-relay CI gates run a one-second real-time version of the isolated-client soak against disposable PostgreSQL 16, local coturn, a localhost API, Vite, and the Ubuntu runner's system Chrome. Direct peers are denied TURN candidates and must report direct paths; forced-relay peers receive TURN and must report relay paths. This proves both topologies and the long-soak code path on every change without spending 30 hosted minutes per commit. A failing run retains its JSON report plus `apps/game-web/build-artifacts/webrtc-browser-smoke-failure.png` for CI artifact upload.

The CI workflow also exposes a manual `network_soak_profile=release-30-minute` dispatch. It runs 30 real-time minutes in both direct and forced-relay modes, binds matchmaking and both retained reports to `github.sha`, verifies them against the provider-equivalent `gw.web-release.v1` attestation, and emits `gw.release-network-soak.v1`. That aggregate hashes both source reports and rejects a short sample, wrong build, non-direct direct profile, non-relay forced profile, non-time-limited relay credentials, failed transport/rollback thresholds, or inconsistent duration evidence. This option is deliberately not selected by push or pull-request builds. It uses only disposable runner-local application infrastructure and does not contact the deployed Gravity Well web, API, database, or TURN service; therefore it is useful exact-release endurance evidence but is not cross-network, two-device, or hosted staging proof.
