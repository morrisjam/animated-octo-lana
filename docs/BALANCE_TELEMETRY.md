...
Normal CI runs this command with `continue-on-error` so development infrastructure, package, and regression checks can remain actionable while the game is intentionally pre-alpha; its JSON and Markdown evidence are still archived. `npm run alpha:local-gate` remains strict, and `.github/workflows/safe-rollout.yml` reruns `ai:alpha-readiness` against the exact release SHA before any provider health, drain, or deployment call. A release therefore cannot use the advisory CI status to bypass the gameplay gate.
