...
- `npm run api:smoke:auth-rate-limit` proves sequential and concurrent database boundaries. `npm run api:smoke:auth-security` proves route ownership and the ticket boundary against a running local API. Neither command contacts Steam when used by CI/local integration.
